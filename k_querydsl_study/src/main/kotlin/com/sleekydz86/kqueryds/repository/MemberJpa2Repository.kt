...
import com.sleekydz86.kqueryds.dto.QMemberTeam2Dto

@Repository
class MemberJpa2Repository(
...
//회원명, 팀명, 나이(ageGoe, ageLoe)
    fun search(condition: MemberSearchCondition): List<MemberTeam2Dto> {
        return queryFactory
            .select(
                QMemberTeam2Dto(
                    member.id,
    ...
    fun findMember(condition: MemberSearchCondition): List<Member> {
        return queryFactory
            .selectFrom(member)
            .leftJoin(member.team, team)
            .where(
