...
class UserLifecycleApplicationService(
    ...
    fun join(command: JoinCommand): User =
        publish(userCommandService.join(command), UserActivityEventType.JOIN)

    fun createByAdmin(command: CreateUserByAdminCommand): User =
        publish(userCommandService.createByAdmin(command), UserActivityEventType.JOIN)
    ...
    fun suspend(command: SuspendUserCommand): User =
        publish(userCommandService.suspend(command), UserActivityEventType.SUSPEND)
    ...
    fun withdraw(command: WithdrawUserCommand): User =
        publish(userCommandService.withdraw(command), UserActivityEventType.WITHDRAW)
    ...
    fun changePassword(command: ChangePasswordWithVerifyCommand): User =
        publish(userCommandService.changePassword(command), UserActivityEventType.PASSWORD_CHANGE)
    ...
    private fun publish(user: User, eventType: UserActivityEventType? = null): User {
        user.id?.let { userId ->
            eventType?.let { userActivityLogService.record(userId, it) }
        }
