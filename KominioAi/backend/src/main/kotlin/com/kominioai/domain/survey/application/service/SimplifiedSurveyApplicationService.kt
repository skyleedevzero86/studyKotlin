...
import com.kominioai.global.service.SimplifiedBusinessMetricsService
...
@Service
@Transactional
class SimplifiedSurveyApplicationService(
    private val surveyRepository: SurveyRepository,
    private val surveyResponseRepository: SurveyResponseRepository,
    private val eventPublisher: EventPublisher,
    private val metricsService: SimplifiedBusinessMetricsService
) : SurveyUseCase {
    ...
    override fun createSurvey(command: CreateSurveyCommand): Mono<SurveyId> {
        ...
        return surveyRepository.save(survey)
            ...
            .doOnSuccess { surveyId ->
                metricsService.recordSurveyCreated(surveyId.value)
            }
    }
    ...
    override fun addQuestion(command: AddQuestionCommand): Mono<QuestionId> {
        return surveyRepository.findById(command.surveyId)
            ...
            .doOnSuccess { questionId ->
                metricsService.recordQuestionAdded(command.surveyId.value, command.type.name, command.order)
            }
    }
    ...
    override fun publishSurvey(command: PublishSurveyCommand): Mono<Void> {
        return surveyRepository.findById(command.surveyId)
            ...
            .doOnSuccess {
                metricsService.recordSurveyPublished(command.surveyId.value, 0)
            }
    }
    ...
    override fun submitResponse(command: SubmitResponseCommand): Mono<ResponseId> {
        return surveyRepository.findById(command.surveyId)
            ...
            .doOnSuccess { responseId ->
                metricsService.recordSurveyResponseSubmitted(command.surveyId.value, command.answers.size)
            }
    }
